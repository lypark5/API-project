...
function EditSpotFunction() {
  ...
  const history = useHistory();
  const dispatch = useDispatch();
  const allSpots = useSelector(state => state.spots.allSpots);  // this can be any of the single spots u last visited
  console.log('allSpots =', allSpots)
  const allSpotsArr = Object.values(allSpots); 
  console.log('allSpotsArr =', allSpotsArr)
  console.log('spotId =', spotId)
  console.log('type spot.id =', typeof allSpotsArr[0].id)  // this is a number
  console.log('type spotId =', typeof spotId)     // this is a string
  const spotBeingEdited = (allSpotsArr.filter(spot => spot.id == spotId))[0]  
  
  console.log('spotBeingEdited =', spotBeingEdited)
  const [country, setCountry] = useState(spotBeingEdited.country)
  const [address, setAddress] = useState(spotBeingEdited.address);
  const [city, setCity] = useState("");
  const [state, setState] = useState("");
  const [lat, setLat] = useState(0);
  const [lng, setLng] = useState(0);
  ...
  useEffect(() => {}, 
    [dispatch, spotBeingEdited, country, address]);
